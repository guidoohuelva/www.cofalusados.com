<?php
class ASTEG_userdatabase
{
	var $db_user;
	//---- CONSTRUCTOR
	public function __construct()
	{
		if (isset($_SESSION['ASTEGFRAMEWORK_USER_ID']))
		{
			include_once (THEME_PATH."plugins/ASTEGplugin.users.php");
			$user_class							= new ASTEGplugin_users();
			$database_array				= $user_class->users_get_database_spec();
			$this->db_user 					= &ADONewConnection('mysql'); 				 														//---- CONNECTION TYPE DEFINITION
			$this->db_user->Connect($database_array['dbhost'],$database_array['dbuser'],$database_array['dbpass'],$database_array['dbname']);
		}
	}
	...
	public function database_create($table, $requestArray)
	{
		...
		$insertQuery	= $this->db_user->GetInsertSQL($selectQuery,$requestArray);
		if ($this->db_user->Execute($insertQuery))
		{
			$returnArray['result']					= true;
			$returnArray['ID']						= $this->db_user->Insert_ID();
		}
